...
import { reactForumPostAction } from "@/lib/actions/forum-actions";

type ReactionOpt = { id: number; name: string; emoji: string; weight: number };

/** Barra de reações de um post do fórum, estilo IPB (idêntica à do artigo): resumo
 * de quem reagiu + botão que revela as opções no hover/clique. */
export function ForumReactionBar({
  ...
  function react(id: number) {
    ...
    setOpen(false);
    start(async () => {
      const res = await reactForumPostAction(postId, id);
      if (!res.ok) {
